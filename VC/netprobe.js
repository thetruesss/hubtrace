// Пробник. Крутится в MAIN world и подсматривает, каким запросом страница
// тянет историю предмета. Запомненный запрос («рецепт») потом повторяется
// для любого другого номера — это один round-trip вместо загрузки всей SPA.
// Наружу ничего не шлём, только postMessage в scanner.js.
(() => {
  ...
  // номер некуда и на каждый запрос приедет история чужого предмета.
  ...
  function scoreCandidate(request, responseText) {
    ...
    // статика и телеметрия не нужны ни при каких раскладах
  ...
  // Версию приложения и склад оператора Hub шлёт в своих же запросах. Они нужны
  // для обращения к известным ручкам напрямую, поэтому забираем из любого
  // обращения, а не только из тех, что годятся в рецепт.
  // x-o3-app-* лежат в глобалке страницы, из MAIN-мира она видна
